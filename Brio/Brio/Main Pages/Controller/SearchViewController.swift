...
class SearchViewController: UIViewController, UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate, SearchTableViewCellDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let userImageUrl = followingUser[indexPath.row].object(forKey: "imageUrl") as? String else {
            return cell
        }
        ...
        cell.userNameLabel.text = followingUser[indexPath.row].object(forKey: "displayName") as? String
        
        // Followボタンを機能させる
        cell.tag = indexPath.row
        cell.delegate = self
        
        if followingUserIds.contains(followingUser[indexPath.row].objectId) {
    ...
    func didTapFollowButton(tableViewCell: UITableViewCell, button: UIButton) {
        let displayName = followingUser[tableViewCell.tag].object(forKey: "displayName") as? String
        let message = displayName! + "をフォローしますか？"
        let alert = UIAlertController(title: "フォロー", message: message, preferredStyle: .alert)
        let okAction = UIAlertAction(title: "OK", style: .default) { (action) in
            self.follow(selectedUser: self.followingUser[tableViewCell.tag])
        }
        ...
        query?.findObjectsInBackground { [weak self] result, error in
            ...
            if error != nil {
                ...
                result?.forEach { item in
                    print(item)
                    guard let nCMBObject = item as? NCMBObject, let user = nCMBObject.object(forKey: "following") as? NCMBUser else {
                        return
                    }
